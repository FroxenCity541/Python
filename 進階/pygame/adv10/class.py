...
def bg_update():
    ...
    bg_roll_x = (bg_roll_x - 1000) % bg_x  # 背景移動
...
def score_update():
    """更新分數"""
    score_sur = score_font.render(str(score), True, Green)
...
while True:
    ...
    if gg:
        ...
        if enemy_random ==0:
            ...
            # pygame.draw.circle(screen, Green, (int(cacti_center_x), int(cacti_center_y)), cacti_detect_r + ds_detect_r, 1)
        else:
            move_ptera()
            gg = is_hit(ds_center_x, ds_center_y, ptera_center_x, ptera_center_y, ptera_detect_r + ds_detect_r)
            # pygame.draw.circle(screen, Green, (int(ptera_center_x), int(ptera_center_y)), ptera_detect_r + ds_detect_r, 1)
